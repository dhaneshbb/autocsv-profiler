...
from tests.utils.test_helpers import MemoryTracker, create_test_csv, get_file_size_mb
...
class TestMemoryPerformance:
    ...
    def test_memory_usage_small_file(self, memory_tracker, sample_csv_small):
        ...
        if memory_info["peak"] is not None:
            assert memory_info["peak"] < 350  # Should be under 350MB
    ...
    def test_memory_usage_medium_file(self, memory_tracker, tmp_path):
        ...
        if memory_info["peak"] is not None:
            # Memory usage should be less than 100x file size
            assert memory_info["peak"] < file_size * 100
    ...
    def test_memory_usage_large_file(self, memory_tracker, tmp_path):
        ...
        _file_size = get_file_size_mb(csv_path)
